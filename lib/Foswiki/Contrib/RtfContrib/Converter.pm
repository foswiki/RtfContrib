...
sub readStrings {
    ...
    my $translationsTopic = $this->{translationsTopic}
      || $Foswiki::cfg{RtfContrib}{Translations};
...
sub genRtf {
    ...
    writeDebug(
"templateWeb=$templateWeb, templateTopic=$templateTopic, template=$template"
    );

    my $rtf =
      Foswiki::Func::readAttachment( $templateWeb, $templateTopic, $template );
    return ( undef,
        "template '$templateWeb.$templateTopic.$template' not found" )
      unless $rtf;
...
sub processTemplate {
    ...
    if ($formObj) {
        $_[0] =~
s/\%FORMFIELD\\{("?.*?"?)\\}\%/$this->handleFormField($formObj, $1)/ge;
    }
    else {
    ...
    $_[0] =~
      s/\%SECTION\\{("?.*?"?)\\}\%/$this->handleTopicKey($topicObj, $1)/ge;
    $_[0] =~
s/\%ATTACHMENT\\{("?.*?"?)\\}\%/$this->handleAttachment($attachmentsObj, $1)/ge;
...
sub handleImage {
    ...
    my ( $photo, $type, $imgWidth, $imgHeight, $errorMsg ) =
      $this->getPhoto($fileName);
    return ( undef, "error while fetching photo: $errorMsg" ) unless $photo;

    $imgWidth = 1
      unless $imgWidth;    # prevent division by zero; should never reach this
    ...
    if ($blipCode) {
     ...
     # SMELL: make positioning more configurable; we only have pos top/right now
        ...
        writeDebug(
            "ratio=$ratio, twipsWidth=$twipsWidth twipsHeight=$twipsHeight");
        writeDebug(
"top=$twipsTop, bottom=$twipsBottom, left=$twipsLeft, right=$twipsRight"
        );
...
sub handleCell {
    ...
    writeDebug(
        "theWeb=$theWeb, theTopic=$theTopic, theRow=$theRow, theCol=$theCol");
...
sub getPhoto {
    ...
    if ( $fileName =~ /^(https?|ftp)/ ) {
        ...
        return ( undef, undef, undef, undef, $response->status_line )
    ...
    my ( $width, $height, $format ) =
      $this->{mage}->Get( 'width', 'height', 'format' );
    $format = lc($format);
